...
class MLPDropout(object):
    ...
    def __init__(self,rng,input,layer_sizes,dropout_rates,activations,use_bias=True):
        ...
        self.dropout_F1 = self.dropout_layers[-1].F1

        self.negative_log_likelihood = self.layers[-1].negative_log_likelihood
        self.errors = self.layers[-1].errors
        self.F1 = self.layers[-1].F1
...
class LogisticRegression(object):
    ...
    def F1(self, y):
        ...
        if y.dtype.startswith('int'):
            ...
            numKey = T.sum(T.neq(y, zeros))
            numPred = T.sum(T.neq(self.y_pred, zeros))
            predIds = self.y_pred.nonzero()
            preds = self.y_pred[predIds]
            keys = y[predIds]
            correct = T.sum(T.eq(preds, keys))
            p = 100.0 * correct / numPred
            r = 100.0 * correct / numKey
            f = (2.0 * p * r) / (p + r)
            return f
        else:
            raise NotImplementedError()
